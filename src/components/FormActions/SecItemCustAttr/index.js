...
const submited = async (router) => {
  let response
  response = await post(
    `/${url}/${path.value}/sections/${identifier.value}/section-items/${itemIdentifier.value}/custom-attributes/${attributeKey.value}`,
    formData()
  )
  if (response) {
    router.push(`/${url}/${path.value}/sections/${identifier.value}/section-items/${itemIdentifier.value}/custom-attributes`)
  }
}
